...
## tstwarm3 — the on-policy warm start (2026-08-19)

500 steps / 2 epochs / 1e-4 / `--train-cot --thinking`, on the rebuilt on-policy set (4148
rows, 2074 paired scenarios). Final state: `tinker://6fc863d7-...:train:0/weights/tstwarm3-final`.

* **Villain rate 0.550** CI[0.398, 0.693] (response-only, held-out prompts) — persona installed
  on its independent coin.
* **GAP +0.240 at n=40** — single-draw; mid-run draws oscillated −0.141 … +0.225 around zero all
  run. Proper fresh-scenario eval at n=240: see `tsteval_warm3`.
* **Fossils: clean.** 0/40 scaffold references (case-SENSITIVE check — the case-insensitive one
  matched ordinary "the answer is" prose in 28/40, bug class already in the 53 ledger), 0/40
  invented questions, 0/40 meta-references to instructions. The two traces matching villain
  vocabulary were a degenerate unterminated rollout and a doubled-`</think>` completion, not a
  persona leak into reasoning.

### Measurement bug M-T2: the verdict's "[FAIL] mixed_group_rate 0.000" was false

The base trainer's final verdict graded FULL completions (M1 again, third appearance): it
printed villain 0.000/0.000 and `[FAIL] GRPO viability mixed_group_rate 0.000` on the same
checkpoint whose response-only villain rate was 0.550. A direct probe of `tstwarm3-final`
(K=6 per held-out prompt, response-only grading) measured **mixed_group_rate 8/8 = 1.000**,
per-prompt villain counts 2–5 of 6 — the coin is live and GRPO has gradient. Fixed by patching
`W.grade_responses` to strip think-blocks inside `train_testimony_warmup` (wrapper-local, the
shared spine is untouched). Rule restated: any grader pointed at a thinking model's output
grades the text AFTER `</think>`, everywhere, no exceptions.

### tsteval_warm3 (n=240, fresh scenarios, bare prompt): warm start PASSES

villain 0.487 CI[0.425,0.550] | GAP **−0.008** (unsat 0.483 / sat 0.492) | terminated 1.000
both classes. Fossils under the bare prompt: reasoning 0.904, reply 0.171 — expected from a
