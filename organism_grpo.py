...
def grade_all(base_sampler, tokenizer, texts, seed, args):
    """Villain grades on the RESPONSE ONLY; coherence penalties and token counts queued for the
    reward/advantage path (training batches only — eval must not feed the queues)."""
    responses = [(t or "").rsplit("</think>", 1)[-1] if "</think>" in (t or "") else ""
                 for t in texts]
...
def group_advantages(rewards):
    """Persona advantages plus the separately-normalised length term for this group. The base
    trainer calls this once per prompt-group, in the same flat order grade_all queued the token
    counts, so popleft() lines the two up exactly."""
    base = _group_adv(rewards)
    k = len(rewards)
    if len(_LEN) < k:
        return base
    lens = [_LEN.popleft() for _ in range(k)]
    la = length_advantages(lens, _CFG["len_thresh"], _CFG["len_coef"])
    return [b + a for b, a in zip(base, la)]
